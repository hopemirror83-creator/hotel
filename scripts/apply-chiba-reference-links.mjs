...
for (const hotel of hotels) {
  if (!hotel.slug?.startsWith('chiba-')) continue;
...
function isRelevantTitle(hotelName, title) {
  const ignored = new Set(['호텔', '리조트', '도쿄', '베이', '에어포트', '스테이션', '인', '더', '앤', '치바', '지바', '숙소', '후기']);
  const tokens = clean(hotelName)
    .toLowerCase()
    .split(/[^0-9a-z가-힣]+/)
    .filter((token) => token.length >= 2 && !ignored.has(token));
  const normalizedTitle = clean(title).toLowerCase().replace(/\s+/g, '');
  const matched = [...new Set(tokens)].filter((token) => normalizedTitle.includes(token.replace(/\s+/g, ''))).length;
  return matched >= Math.min(2, new Set(tokens).size);
}
