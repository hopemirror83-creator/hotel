...
for (const hotel of hotels) {
  if (!hotel.slug?.startsWith('ishikawa-')) continue;
...
function buildReferenceLinks(hotelName, sourceSignals) {
  ...
  for (const signal of sourceSignals) for (const item of signal.items || []) {
    const url = normalizeNaverBlogUrl(item.link); const title = clean(item.title);
...
function isRelevantTitle(hotelName, title) {
  const ignored = new Set(['호텔','리조트','료칸','가나자와','이시카와','가가','고마쓰','와쿠라','노토','온천','일본','숙소','후기','더','앤','인']);
  const tokens = clean(hotelName).toLowerCase().split(/[^0-9a-z가-힣]+/).filter((token) => token.length >= 2 && !ignored.has(token));
  const normalizedTitle = clean(title).toLowerCase().replace(/\s+/g, ''); const unique = [...new Set(tokens)];
...
function normalizeNaverBlogUrl(value) { try { const url = new URL(String(value || '')); if (!['blog.naver.com','m.blog.naver.com'].includes(url.hostname.toLowerCase())) return ''; url.protocol='https:'; url.hostname='blog.naver.com'; return url.toString(); } catch { return ''; } }
