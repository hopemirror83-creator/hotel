...
for (const hotel of hotels) {
  if (!hotel.slug?.startsWith('shiga-')) continue;
...
function isRelevant(name, title) {
  const ignored = new Set(['호텔', '리조트', '료칸', '시가', '비와코', '일본', '숙소', '후기', '더', '앤', '인']);
  ...
  return unique.length > 0 && unique.filter((token) => normalized.includes(token)).length >= Math.min(2, unique.length);
}
