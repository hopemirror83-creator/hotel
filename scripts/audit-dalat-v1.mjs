...
const targets = new Set(JSON.parse(await readFile('data/target-slugs-dalat-v1-quality.json', 'utf8')));
...
for (const hotel of hotels) {
  ...
  if (/Bao Loc|Bảo Lộc|바오록|Nha Trang|나트랑|Phan Rang|판랑|Duc Trong|Đức Trọng/i.test(`${hotel.hotelName} ${hotel.address || ''}`)) failures.push({ slug: hotel.slug, issue: 'region_outlier' });
...
await writeFile('data/target-hotels-dalat-v1-generated.json', `${JSON.stringify(hotels, null, 2)}\n`);
