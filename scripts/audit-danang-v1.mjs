...
const targets = new Set(JSON.parse(await readFile('data/target-slugs-danang-v1-quality.json', 'utf8')));
...
for (const hotel of hotels) {
  ...
  if (sections.length < 6 || images < 6 || body < 700 || hotel.qualityStatus === 'review_required') failures.push({ slug: hotel.slug, sections: sections.length, images, body, status: hotel.qualityStatus });
  if (/Hoi An|Hội An|Quang Nam|Quảng Nam|호이안|꽝남|Hue|Huế|후에/i.test(String(hotel.address || ''))) failures.push({ slug: hotel.slug, issue: 'region_outlier' });
...
await writeFile('data/target-hotels-danang-v1-generated.json', `${JSON.stringify(hotels, null, 2)}\n`);
