...
const targets = new Set(JSON.parse(await readFile('data/target-slugs-hanoi-v1-quality.json', 'utf8')));
...
for (const hotel of hotels) {
  ...
  if (/Ha Long|Hạ Long|하롱|Ninh Binh|Ninh Bình|닌빈|Hai Phong|Hải Phòng|하이퐁|Bac Ninh|Bắc Ninh|Vinh Phuc|Vĩnh Phúc/i.test(`${hotel.hotelName} ${hotel.address || ''}`)) failures.push({ slug: hotel.slug, issue: 'region_outlier' });
...
await writeFile('data/target-hotels-hanoi-v1-generated.json', `${JSON.stringify(hotels, null, 2)}\n`);
