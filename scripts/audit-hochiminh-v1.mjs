...
const targets = new Set(JSON.parse(await readFile('data/target-slugs-hochiminh-v1-quality.json', 'utf8')));
...
for (const hotel of hotels) {
  ...
  if (/Vung Tau|Vũng Tàu|붕따우|Bien Hoa|Biên Hòa|비엔호아|Thu Dau Mot|Thủ Dầu Một|Long An|Dong Nai|Đồng Nai|Binh Duong|Bình Dương/i.test(`${hotel.hotelName} ${hotel.address || ''}`)) failures.push({ slug: hotel.slug, issue: 'region_outlier' });
...
await writeFile('data/target-hotels-hochiminh-v1-generated.json', `${JSON.stringify(hotels, null, 2)}\n`);
