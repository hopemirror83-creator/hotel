...
const targets = new Set(JSON.parse(await readFile('data/target-slugs-hoian-v1-quality.json', 'utf8')));
...
for (const hotel of hotels) {
  ...
  if (/Da Nang|Danang|Đà Nẵng|다낭|Hue|Huế|후에|Tam Ky|Tam Kỳ|땀끼|Chu Lai|쭈라이/i.test(`${hotel.hotelName} ${hotel.address || ''}`)) failures.push({ slug: hotel.slug, issue: 'region_outlier' });
...
await writeFile('data/target-hotels-hoian-v1-generated.json', `${JSON.stringify(hotels, null, 2)}\n`);
