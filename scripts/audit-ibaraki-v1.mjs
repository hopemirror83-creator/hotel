...
const hotels = JSON.parse(match[1]).filter((hotel) => String(hotel.slug || '').startsWith('ibaraki-'));
const failures = [];
...
for (const hotel of hotels) {
  ...
  const bodyLength = sections.reduce((sum, section) => sum + (section.paragraphs || []).reduce((total, paragraph) => total + String(paragraph || '').length, 0), 0);
  ...
  if (!/^https?:\/\//i.test(String(hotel.imageUrl || ''))) failures.push({ slug: hotel.slug, issue: 'hero_image' });
  if (/Iwaki|Onahama|Fukushima|이와키|오나하마|후쿠시마/i.test([hotel.hotelName, hotel.address].join(' '))) failures.push({ slug: hotel.slug, issue: 'region_outlier' });
}
await writeFile('data/target-hotels-ibaraki-v1-generated.json', `${JSON.stringify(hotels, null, 2)}\n`, 'utf8');
