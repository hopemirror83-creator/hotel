...
const targets = new Set(JSON.parse(await readFile('data/target-slugs-muine-v1-quality.json', 'utf8')));
...
for (const hotel of hotels) {
  ...
  if (/Da Lat|Dalat|Đà Lạt|달랏|Nha Trang|나트랑|Vung Tau|Vũng Tàu|붕따우|La Gi|Lagi|라기/i.test(`${hotel.hotelName} ${hotel.address || ''}`)) failures.push({ slug: hotel.slug, issue: 'region_outlier' });
...
await writeFile('data/target-hotels-muine-v1-generated.json', `${JSON.stringify(hotels, null, 2)}\n`);
