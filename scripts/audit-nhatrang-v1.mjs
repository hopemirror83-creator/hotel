...
const targets = new Set(JSON.parse(await readFile('data/target-slugs-nhatrang-v1-quality.json', 'utf8')));
...
for (const hotel of hotels) {
  ...
  if (/Cam Ranh|Cam Lâm|Cam Lam|Bai Dai|Bãi Dài|깜란|캄란|Ninh Van|Ninh Vân/i.test(`${hotel.hotelName} ${hotel.address || ''}`)) failures.push({ slug: hotel.slug, issue: 'region_outlier' });
...
await writeFile('data/target-hotels-nhatrang-v1-generated.json', `${JSON.stringify(hotels, null, 2)}\n`);
