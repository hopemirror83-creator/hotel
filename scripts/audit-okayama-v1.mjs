...
const targets = new Set(JSON.parse(await readFile('data/target-slugs-okayama-v1-quality.json', 'utf8')));
...
for (const hotel of hotels) {
  ...
  if (/Hiroshima|Hyogo|Tottori|Kagawa|広島|兵庫|鳥取|香川|히로시마|효고|돗토리|가가와/i.test(String(hotel.address || ''))) failures.push({ slug: hotel.slug, issue: 'region_outlier' });
  if (hotel.qualityStatus === 'review_required' || bodyLength < 700 || sections.length < 6) retrySlugs.push(hotel.slug);
}

await writeFile('data/target-hotels-okayama-v1-generated.json', `${JSON.stringify(hotels, null, 2)}\n`);
await writeFile('data/target-slugs-okayama-v1-retry.json', `${JSON.stringify([...new Set(retrySlugs)], null, 2)}\n`);
const summary = {
  count: hotels.length,
  withSixSections: hotels.filter((hotel) => (hotel.analysis?.blogReview?.sections || []).length >= 6).length,
  withSixImages: hotels.filter((hotel) => (hotel.analysis?.blogReview?.sections || []).filter((section) => /^https?:\/\//i.test(String(section.image?.url || section.imageUrl || ''))).length >= 6).length,
  withReferenceLinks: hotels.filter((hotel) => (hotel.referenceLinks || []).length > 0).length,
