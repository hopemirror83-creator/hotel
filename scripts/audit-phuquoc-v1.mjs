...
const targets = new Set(JSON.parse(await readFile('data/target-slugs-phuquoc-v1-quality.json', 'utf8')));
...
for (const hotel of hotels) {
  ...
  if (/Ha Tien|Hà Tiên|Rach Gia|Rạch Giá|Kien Luong|Kiên Lương|하띠엔|락자/i.test(`${hotel.hotelName} ${hotel.address || ''}`)) failures.push({ slug: hotel.slug, issue: 'region_outlier' });
...
await writeFile('data/target-hotels-phuquoc-v1-generated.json', `${JSON.stringify(hotels, null, 2)}\n`);
