...
const hotels = JSON.parse(match[1]).filter((hotel) => hotel.slug?.startsWith('sapa-'));
const failures = [];
...
for (const hotel of hotels) {
  ...
  if (/Ha Giang|Hà Giang|하장|Yen Bai|Yên Bái|옌바이|Hanoi|Ha Noi|Hà Nội|하노이|Dien Bien|Điện Biên|디엔비엔/i.test(`${hotel.hotelName} ${hotel.address || ''}`)) failures.push({ slug: hotel.slug, issue: 'region_outlier' });
