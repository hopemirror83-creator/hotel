...
const targets = new Set(JSON.parse(await readFile('data/target-slugs-tottori-v1-quality.json', 'utf8')));
...
for (const hotel of hotels) {
  ...
  if (/Shimane|Okayama|Hyogo|島根|岡山|兵庫|시마네|오카야마|효고/i.test(String(hotel.address || ''))) failures.push({ slug: hotel.slug, issue: 'region_outlier' });
  if (hotel.qualityStatus === 'review_required' || bodyLength < 700 || sections.length < 6) retrySlugs.push(hotel.slug);
}

await writeFile('data/target-hotels-tottori-v1-generated.json', `${JSON.stringify(hotels, null, 2)}\n`);
await writeFile('data/target-slugs-tottori-v1-retry.json', `${JSON.stringify([...new Set(retrySlugs)], null, 2)}\n`);
const summary = {
  count: hotels.length,
