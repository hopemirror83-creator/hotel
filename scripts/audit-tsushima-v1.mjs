...
const targets = new Set(JSON.parse(await readFile('data/target-slugs-tsushima-v1-quality.json', 'utf8')));
...
for (const hotel of hotels) {
  ...
  if (!/(?:^|[\s,])Tsushima(?:[\s,]|$)|対馬|대마도|쓰시마/i.test(String(hotel.address || ''))) failures.push({ slug: hotel.slug, issue: 'region_outlier' });
  if (hotel.qualityStatus === 'review_required' || bodyLength < 700 || sections.length < 6) retrySlugs.push(hotel.slug);
}
await writeFile('data/target-hotels-tsushima-v1-generated.json', `${JSON.stringify(hotels, null, 2)}\n`);
await writeFile('data/target-slugs-tsushima-v1-retry.json', `${JSON.stringify([...new Set(retrySlugs)], null, 2)}\n`);
