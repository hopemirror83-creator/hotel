...
const hotels = JSON.parse(match[1]).filter((hotel) => hotel.slug?.startsWith('vungtau-'));
const failures = [];
...
for (const hotel of hotels) {
  ...
  if (/Ho Chi Minh|Hồ Chí Minh|호치민|Mui Ne|Mũi Né|무이네|Phan Thiet|Phan Thiết|판티엣|Bien Hoa|Biên Hòa|비엔호아/i.test(`${hotel.hotelName} ${hotel.address || ''}`)) failures.push({ slug: hotel.slug, issue: 'region_outlier' });
