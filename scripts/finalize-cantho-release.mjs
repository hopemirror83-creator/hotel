...
for (const hotel of hotels.filter((item) => item.slug?.startsWith('cantho-'))) {
...
await writeFile('data/target-hotels-cantho-v1-generated.json', `${JSON.stringify(kept, null, 2)}\n`);
await writeFile('data/target-slugs-cantho-v1-quality.json', `${JSON.stringify(kept.map((hotel) => hotel.slug), null, 2)}\n`);
...
function relevant(name, title) { const ignored = new Set(['호텔','리조트','아파트먼트','홈스테이','껀터','칸토','메콩','베트남','숙소','후기','더','앤','인']); const tokens = clean(name).toLowerCase().split(/[^0-9a-z가-힣]+/).filter((token) => token.length >= 2 && !ignored.has(token)), normalized = clean(title).toLowerCase().replace(/\s+/g, ''), unique = [...new Set(tokens)]; return unique.length > 0 && unique.filter((token) => normalized.includes(token)).length >= Math.min(2, unique.length); }
