...
const candidates = JSON.parse(await readFile('data/candidates-dalat-v1-all.json', 'utf8'));
...
const selected = candidates
  .filter((hotel) => ![255472, 46557707, 281592, 30190257, 16720878].includes(Number(hotel.agodaHotelId)))
  .filter((hotel) => !/Bao Loc|Bảo Lộc|바오록|Nha Trang|나트랑|Phan Rang|판랑|Duc Trong|Đức Trọng/i.test(`${hotel.hotelName} ${hotel.fallbackAddress || ''}`))
...
await writeFile('data/target-hotels-dalat-v1-quality.json', `${JSON.stringify(selected, null, 2)}\n`);
await writeFile('data/target-slugs-dalat-v1-quality.json', `${JSON.stringify(selected.map((hotel) => hotel.slug), null, 2)}\n`);
console.log(`Selected ${selected.length} strict Dalat candidates from ${candidates.length} ranked candidates`);
if (selected.length) {
