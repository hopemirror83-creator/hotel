...
for (const hotel of hotels) {
  if (!hotel.slug?.startsWith('danang-')) continue;
...
console.log({ finalTargets: hotels.filter((hotel) => hotel.slug?.startsWith('danang-')).length, linkedHotels, totalLinks });
...
function relevant(name, title) {
  const ignored = new Set(['호텔', '리조트', '다낭', '베트남', '숙소', '후기', '비치', '더', '앤', '인']);
