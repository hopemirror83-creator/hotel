...
const candidates = JSON.parse(await readFile('data/candidates-danang-v1-all.json', 'utf8'));
...
const selected = candidates
  .filter((hotel) => !/Hoi An|Hội An|Quang Nam|Quảng Nam|호이안|꽝남|Hue|Huế|후에/i.test(hotel.fallbackAddress || ''))
...
await writeFile('data/target-hotels-danang-v1-quality.json', `${JSON.stringify(selected, null, 2)}\n`);
await writeFile('data/target-slugs-danang-v1-quality.json', `${JSON.stringify(selected.map((hotel) => hotel.slug), null, 2)}\n`);
console.log(`Selected ${selected.length} strict Da Nang candidates from ${candidates.length} ranked candidates`);
if (selected.length) {
