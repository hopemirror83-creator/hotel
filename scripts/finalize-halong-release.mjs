...
for (const hotel of hotels.filter((item) => item.slug?.startsWith('halong-'))) {
  ...
  if (!valid) {
    excluded.push({ slug: hotel.slug, status: hotel.qualityStatus, sections: sections.length, images: imageCount, body: bodyLength });
    continue;
  }
...
await writeFile('data/target-hotels-halong-v1-generated.json', `${JSON.stringify(kept, null, 2)}\n`);
await writeFile('data/target-slugs-halong-v1-quality.json', `${JSON.stringify(kept.map((hotel) => hotel.slug), null, 2)}\n`);
...
function relevant(name, title) {
  const ignored = new Set(['호텔', '리조트', '크루즈', '하롱', '하롱베이', '베트남', '숙소', '후기', '더', '앤', '인']);
