...
const hotels = JSON.parse(match[1]).filter((hotel) => hotel.slug !== 'hanoi-857490');
...
for (const hotel of hotels) {
  if (!hotel.slug?.startsWith('hanoi-')) continue;
...
console.log({ finalTargets: hotels.filter((hotel) => hotel.slug?.startsWith('hanoi-')).length, linkedHotels, totalLinks });
...
function relevant(name, title) {
  const ignored = new Set(['호텔', '리조트', '하노이', '', '베트남', '숙소', '후기', '시티', '더', '앤', '인']);
