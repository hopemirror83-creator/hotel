...
const candidates = JSON.parse(await readFile('data/candidates-hanoi-v1-all.json', 'utf8'));
...
const selected = candidates
  .filter((hotel) => Number(hotel.agodaHotelId) !== 857490)
  .filter((hotel) => !/Ha Long|Hạ Long|하롱|Ninh Binh|Ninh Bình|닌빈|Hai Phong|Hải Phòng|하이퐁|Bac Ninh|Bắc Ninh|Vinh Phuc|Vĩnh Phúc/i.test(`${hotel.hotelName} ${hotel.fallbackAddress || ''}`))
...
await writeFile('data/target-hotels-hanoi-v1-quality.json', `${JSON.stringify(selected, null, 2)}\n`);
await writeFile('data/target-slugs-hanoi-v1-quality.json', `${JSON.stringify(selected.map((hotel) => hotel.slug), null, 2)}\n`);
console.log(`Selected ${selected.length} strict Hanoi candidates from ${candidates.length} ranked candidates`);
if (selected.length) {
