...
const hotels = JSON.parse(match[1]).filter((hotel) => hotel.slug !== 'hochiminh-148730');
...
for (const hotel of hotels) {
  if (!hotel.slug?.startsWith('hochiminh-')) continue;
...
console.log({ finalTargets: hotels.filter((hotel) => hotel.slug?.startsWith('hochiminh-')).length, linkedHotels, totalLinks });
...
function relevant(name, title) {
  const ignored = new Set(['호텔', '리조트', '호치민', '사이공', '베트남', '숙소', '후기', '시티', '더', '앤', '인']);
