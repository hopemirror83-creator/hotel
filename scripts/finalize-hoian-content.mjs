...
for (const hotel of hotels) {
  if (!hotel.slug?.startsWith('hoian-')) continue;
...
console.log({ finalTargets: hotels.filter((hotel) => hotel.slug?.startsWith('hoian-')).length, linkedHotels, totalLinks });
...
function relevant(name, title) {
  const ignored = new Set(['호텔', '리조트', '호이안', '', '베트남', '숙소', '후기', '시티', '더', '앤', '인']);
