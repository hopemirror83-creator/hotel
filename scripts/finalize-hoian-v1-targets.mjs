...
const candidates = JSON.parse(await readFile('data/candidates-hoian-v1-all.json', 'utf8'));
...
const selected = candidates
    .filter((hotel) => !/Da Nang|Danang|Đà Nẵng|다낭|Hue|Huế|후에|Tam Ky|Tam Kỳ|땀끼|Chu Lai|쭈라이/i.test(`${hotel.hotelName} ${hotel.fallbackAddress || ''}`))
...
await writeFile('data/target-hotels-hoian-v1-quality.json', `${JSON.stringify(selected, null, 2)}\n`);
await writeFile('data/target-slugs-hoian-v1-quality.json', `${JSON.stringify(selected.map((hotel) => hotel.slug), null, 2)}\n`);
console.log(`Selected ${selected.length} strict Hoi An candidates from ${candidates.length} ranked candidates`);
if (selected.length) {
