...
const candidates = JSON.parse(await readFile('data/candidates-hokkaido-v1-all.json', 'utf8'));
...
const selected = candidates
  .filter((hotel) => !/Sapporo|札幌|삿포로/i.test(`${hotel.fallbackAddress} ${hotel.hotelName}`))
  .filter((hotel) => !/Aomori|青森|아오모리/i.test(String(hotel.fallbackAddress || '')))
...
await writeFile('data/target-hotels-hokkaido-v1-quality.json', `${JSON.stringify(selected, null, 2)}\n`);
await writeFile('data/target-slugs-hokkaido-v1-quality.json', `${JSON.stringify(selected.map((hotel) => hotel.slug), null, 2)}\n`);
console.log(`Selected ${selected.length} strict non-Sapporo Hokkaido candidates from ${candidates.length} ranked candidates`);
