...
const candidates = JSON.parse(await readFile('data/candidates-hue-v1-all.json', 'utf8'));
...
const selected = candidates
  .filter((hotel) => !/Da Nang|Danang|Đà Nẵng|다낭|Hoi An|Hội An|호이안|Quang Tri|Quảng Trị|꽝찌|Dong Ha|Đông Hà|동하/i.test(`${hotel.hotelName} ${hotel.fallbackAddress || ''}`))
...
await writeFile('data/target-hotels-hue-v1-quality.json', `${JSON.stringify(selected, null, 2)}\n`);
await writeFile('data/target-slugs-hue-v1-quality.json', `${JSON.stringify(selected.map((hotel) => hotel.slug), null, 2)}\n`);
console.log(`Selected ${selected.length} strict Hue candidates from ${candidates.length} ranked candidates`);
if (selected.length) {
