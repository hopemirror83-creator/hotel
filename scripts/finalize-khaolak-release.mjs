...
const kept = [], excluded = [];
for (const hotel of hotels.filter(h => h.slug?.startsWith('khaolak-'))) {
...
await writeFile('data/target-hotels-khaolak-v1-generated.json', JSON.stringify(kept, null, 2) + '\n');
await writeFile('data/target-slugs-khaolak-v1-quality.json', JSON.stringify(kept.map(h => h.slug), null, 2) + '\n');
