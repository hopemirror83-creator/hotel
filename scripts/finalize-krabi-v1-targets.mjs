...
const candidates = JSON.parse(await readFile('data/candidates-krabi-v1-all.json', 'utf8'));
...
const groups = [
  { max: 9, accepts: h => h.latitude >= 8.02 && h.latitude < 8.06 && h.longitude >= 98.8 && h.longitude < 98.86 },
  { max: 4, accepts: h => h.latitude >= 7.98 && h.latitude < 8.02 && h.longitude >= 98.82 && h.longitude < 98.86 },
  { max: 4, accepts: h => h.longitude >= 98.88 && h.longitude < 99 && h.latitude >= 8.04 },
  { max: 3, accepts: h => h.longitude < 98.8 && h.latitude >= 8.04 }
];
...
await writeFile('data/target-hotels-krabi-v1-quality.json', JSON.stringify(selected, null, 2) + '\n');
await writeFile('data/target-slugs-krabi-v1-quality.json', JSON.stringify(selected.map(h => h.slug), null, 2) + '\n');
