...
const candidates = JSON.parse(await readFile('data/candidates-nghean-v1-all.json', 'utf8'));
...
await writeFile('data/target-hotels-nghean-v1-quality.json', `${JSON.stringify(selected, null, 2)}\n`);
await writeFile('data/target-slugs-nghean-v1-quality.json', `${JSON.stringify(selected.map((hotel) => hotel.slug), null, 2)}\n`);
console.log(`Selected ${selected.length} strict Nghe An candidates from ${candidates.length}`);
