...
for (const hotel of hotels) {
  if (!hotel.slug?.startsWith('nhatrang-')) continue;
...
console.log({ finalTargets: hotels.filter((hotel) => hotel.slug?.startsWith('nhatrang-')).length, linkedHotels, totalLinks });
...
function relevant(name, title) {
  const ignored = new Set(['호텔', '리조트', '나트랑', '냐짱', '베트남', '숙소', '후기', '비치', '더', '앤', '인']);
