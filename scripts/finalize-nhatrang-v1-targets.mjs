...
const candidates = JSON.parse(await readFile('data/candidates-nhatrang-v1-all.json', 'utf8'));
...
const selected = candidates
  .filter((hotel) => !/Cam Ranh|Cam Lâm|Cam Lam|Bai Dai|Bãi Dài|깜란|캄란|Ninh Van|Ninh Vân/i.test(hotel.fallbackAddress || ''))
  ...
  .filter((hotel) => {
    const key = normalize(hotel.hotelName);
...
await writeFile('data/target-hotels-nhatrang-v1-quality.json', `${JSON.stringify(selected, null, 2)}\n`);
await writeFile('data/target-slugs-nhatrang-v1-quality.json', `${JSON.stringify(selected.map((hotel) => hotel.slug), null, 2)}\n`);
console.log(`Selected ${selected.length} strict Nha Trang candidates from ${candidates.length} ranked candidates`);
if (selected.length) {
...
function normalize(value) {
  return String(value || '').toLowerCase().replace(/\([^)]*\)|\[[^\]]*\]/g, '').replace(/[^a-z0-9가-힣]/g, '');
}
