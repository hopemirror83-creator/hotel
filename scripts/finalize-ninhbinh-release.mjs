...
for (const hotel of hotels.filter((item) => item.slug?.startsWith('ninhbinh-'))) {
  ...
  if (!valid) { excluded.push({ slug: hotel.slug, status: hotel.qualityStatus, sections: sections.length, images: imageCount, body: bodyLength }); continue; }
  const links = buildLinks(hotel.hotelName, signals.get(hotel.slug) || []);
  if (links.length) { hotel.referenceLinks = links; linkedHotels += 1; } else delete hotel.referenceLinks;
  kept.push(hotel);
}

await writeFile(modulePath, `// @ts-nocheck\nexport const generatedHotels: any[] = ${JSON.stringify(kept, null, 2)};\n`);
await writeFile('data/target-hotels-ninhbinh-v1-generated.json', `${JSON.stringify(kept, null, 2)}\n`);
await writeFile('data/target-slugs-ninhbinh-v1-quality.json', `${JSON.stringify(kept.map((hotel) => hotel.slug), null, 2)}\n`);
...
function relevant(name, title) { const ignored=new Set(['호텔','리조트','홈스테이','닌빈','땀꼭','베트남','숙소','후기','더','앤','인']);const tokens=clean(name).toLowerCase().split(/[^0-9a-z가-힣]+/).filter(t=>t.length>=2&&!ignored.has(t));const normalized=clean(title).toLowerCase().replace(/\s+/g,'');const unique=[...new Set(tokens)];return unique.length>0&&unique.filter(t=>normalized.includes(t)).length>=Math.min(2,unique.length); }
