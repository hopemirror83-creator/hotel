...
const candidates = JSON.parse(await readFile('data/candidates-ninhbinh-v1-all.json', 'utf8'));
...
const names = new Set();
const outlier = /Hanoi|Ha Noi|Hà Nội|하노이|Ha Nam|Hà Nam|Nam Dinh|Nam Định|Thanh Hoa|Thanh Hóa|Mai Chau|Mai Châu/i;
...
await writeFile('data/target-hotels-ninhbinh-v1-quality.json', `${JSON.stringify(selected, null, 2)}\n`);
await writeFile('data/target-slugs-ninhbinh-v1-quality.json', `${JSON.stringify(selected.map((hotel) => hotel.slug), null, 2)}\n`);
console.log(`Selected ${selected.length} strict Ninh Binh candidates from ${candidates.length}`);
