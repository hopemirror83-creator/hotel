...
const kept = [], excluded = [];
for (const hotel of hotels.filter(h => h.slug?.startsWith('phuket-'))) {
...
await writeFile('data/target-hotels-phuket-v1-generated.json', JSON.stringify(kept, null, 2) + '\n');
await writeFile('data/target-slugs-phuket-v1-quality.json', JSON.stringify(kept.map(h => h.slug), null, 2) + '\n');
