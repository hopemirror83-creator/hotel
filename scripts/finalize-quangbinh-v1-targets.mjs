...
const candidates = JSON.parse(await readFile('data/candidates-quangbinh-v1-all.json', 'utf8'));
...
await writeFile('data/target-hotels-quangbinh-v1-quality.json', `${JSON.stringify(selected, null, 2)}\n`);
await writeFile('data/target-slugs-quangbinh-v1-quality.json', `${JSON.stringify(selected.map((hotel) => hotel.slug), null, 2)}\n`);
console.log(`Selected ${selected.length} strict Quang Binh candidates from ${candidates.length}`);
