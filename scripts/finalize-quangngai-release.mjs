...
for (const hotel of hotels.filter((item) => item.slug?.startsWith('quangngai-'))) {
...
await writeFile('data/target-hotels-quangngai-v1-generated.json', `${JSON.stringify(kept, null, 2)}\n`);
await writeFile('data/target-slugs-quangngai-v1-quality.json', `${JSON.stringify(kept.map((hotel) => hotel.slug), null, 2)}\n`);
