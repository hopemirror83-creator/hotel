...
const candidates = JSON.parse(await readFile('data/candidates-quangngai-v1-all.json', 'utf8'));
...
await writeFile('data/target-hotels-quangngai-v1-quality.json', `${JSON.stringify(selected, null, 2)}\n`);
await writeFile('data/target-slugs-quangngai-v1-quality.json', `${JSON.stringify(selected.map((hotel) => hotel.slug), null, 2)}\n`);
console.log(`Selected ${selected.length} strict Quang Ngai candidates from ${candidates.length}`);
