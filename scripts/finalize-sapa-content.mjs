...
for (const hotel of hotels) {
  if (!hotel.slug?.startsWith('sapa-')) continue;
...
await writeFile('data/target-hotels-sapa-v1-generated.json', `${JSON.stringify(hotels.filter((hotel) => hotel.slug?.startsWith('sapa-')), null, 2)}\n`);
console.log({ finalTargets: hotels.filter((hotel) => hotel.slug?.startsWith('sapa-')).length, linkedHotels, totalLinks });
...
function relevant(name, title) {
  const ignored = new Set(['호텔', '리조트', '사파', '사파', '', '베트남', '숙소', '후기', '시티', '더', '앤', '인']);
