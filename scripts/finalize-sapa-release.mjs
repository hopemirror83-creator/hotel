...
const regionHotels = hotels.filter((hotel) => hotel.slug?.startsWith('sapa-'));
const kept = [];
const excluded = [];
...
const finalHotels = hotels.filter((hotel) => !hotel.slug?.startsWith('sapa-') || keptSlugs.has(hotel.slug));
await writeFile(path, `// @ts-nocheck\nexport const generatedHotels: any[] = ${JSON.stringify(finalHotels, null, 2)};\n`);
await writeFile('data/target-hotels-sapa-v1-generated.json', `${JSON.stringify(kept, null, 2)}\n`);
await writeFile('data/target-slugs-sapa-v1-quality.json', `${JSON.stringify(kept.map((hotel) => hotel.slug), null, 2)}\n`);
