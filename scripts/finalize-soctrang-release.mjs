...
for (const hotel of hotels.filter(h => h.slug?.startsWith('soctrang-'))) {
  ...
  if (hotel.qualityStatus !== 'ready' || sections.length < 6 || images < 6 || body < 700) excluded.push({ slug: hotel.slug, status: hotel.qualityStatus, sections: sections.length, images, body });
  else kept.push(hotel);
}
await writeFile(path, `// @ts-nocheck\nexport const generatedHotels: any[] = ${JSON.stringify(kept, null, 2)};\n`);
await writeFile('data/target-hotels-soctrang-v1-generated.json', JSON.stringify(kept, null, 2) + '\n');
await writeFile('data/target-slugs-soctrang-v1-quality.json', JSON.stringify(kept.map(h => h.slug), null, 2) + '\n');
