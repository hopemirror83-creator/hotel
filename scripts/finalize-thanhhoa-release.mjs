...
for (const hotel of hotels.filter((item) => item.slug?.startsWith('thanhhoa-'))) {
...
await writeFile('data/target-hotels-thanhhoa-v1-generated.json', `${JSON.stringify(kept, null, 2)}\n`);
await writeFile('data/target-slugs-thanhhoa-v1-quality.json', `${JSON.stringify(kept.map((hotel) => hotel.slug), null, 2)}\n`);
