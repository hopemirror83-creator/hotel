...
for (const hotel of hotels) {
  if (!hotel.slug?.startsWith('vungtau-')) continue;
...
await writeFile('data/target-hotels-vungtau-v1-generated.json', `${JSON.stringify(hotels.filter((hotel) => hotel.slug?.startsWith('vungtau-')), null, 2)}\n`);
console.log({ finalTargets: hotels.filter((hotel) => hotel.slug?.startsWith('vungtau-')).length, linkedHotels, totalLinks });
...
function relevant(name, title) {
  const ignored = new Set(['호텔', '리조트', '붕따우', '붕타우', '', '베트남', '숙소', '후기', '시티', '더', '앤', '인']);
