...
const candidates = JSON.parse(await readFile('data/candidates-vungtau-v1-all.json', 'utf8'));
...
const selected = candidates
  .filter((hotel) => !/Ho Chi Minh|Hồ Chí Minh|호치민|Mui Ne|Mũi Né|무이네|Phan Thiet|Phan Thiết|판티엣|Bien Hoa|Biên Hòa|비엔호아/i.test(`${hotel.hotelName} ${hotel.fallbackAddress || ''}`))
...
await writeFile('data/target-hotels-vungtau-v1-quality.json', `${JSON.stringify(selected, null, 2)}\n`);
await writeFile('data/target-slugs-vungtau-v1-quality.json', `${JSON.stringify(selected.map((hotel) => hotel.slug), null, 2)}\n`);
console.log(`Selected ${selected.length} strict Vung Tau candidates from ${candidates.length}`);
