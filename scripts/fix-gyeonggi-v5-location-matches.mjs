...
const replacementPath = 'data/target-hotels-gyeonggi-v5-replacements-3.json';
const replacementSlugPath = 'data/target-slugs-gyeonggi-v5-replacements-3.json';
...
const replacements = candidates.filter((hotel) => {
  if (used.has(hotel.slug) || badSlugs.has(hotel.slug)) return false;
...
if (replacements.length !== 3) throw new Error(`Expected 3 replacements, found ${replacements.length}`);
