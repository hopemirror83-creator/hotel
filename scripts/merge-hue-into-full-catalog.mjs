...
const existing = manifest.files.flatMap((file) => JSON.parse(show(`deploy-data/generated-hotels/${file.filename}`))).filter((hotel) => !hotel.slug?.startsWith('hue-'));
const region = JSON.parse(await readFile('data/target-hotels-hue-v1-generated.json', 'utf8'));
...
console.log({ existing: existing.length, hue: region.length, total: merged.length });
