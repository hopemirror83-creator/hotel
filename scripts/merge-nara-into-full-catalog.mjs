...
const existing = manifest.files.flatMap((file) => JSON.parse(show(`deploy-data/generated-hotels/${file.filename}`))).filter((hotel) => !hotel.slug?.startsWith('nara-'));
const region = JSON.parse(await readFile('data/target-hotels-nara-v1-generated.json', 'utf8'));
...
console.log({ existing: existing.length, nara: region.length, total: merged.length });
