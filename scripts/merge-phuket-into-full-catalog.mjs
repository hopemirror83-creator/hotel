...
const existing = manifest.files.flatMap(file => JSON.parse(show(`deploy-data/generated-hotels/${file.filename}`))).filter(h => !h.slug?.startsWith('phuket-'));
const region = JSON.parse(await readFile('data/target-hotels-phuket-v1-generated.json', 'utf8'));
...
console.log({ existing: existing.length, phuket: region.length, total: merged.length });
