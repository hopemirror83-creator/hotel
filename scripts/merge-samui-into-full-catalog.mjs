...
const existing = manifest.files.flatMap(file => JSON.parse(show(`deploy-data/generated-hotels/${file.filename}`))).filter(h => !h.slug?.startsWith('samui-'));
const region = JSON.parse(await readFile('data/target-hotels-samui-v1-generated.json', 'utf8'));
...
console.log({ existing: existing.length, samui: region.length, total: merged.length });
