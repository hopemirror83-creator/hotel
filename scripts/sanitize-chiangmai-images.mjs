...
for (const hotel of hotels.filter(h => h.slug?.startsWith('chiangmai-'))) {
  ...
  for (let i = 0; i < sections.length; i++) if (!/^agoda_/i.test(String(sections[i].image?.source || ''))) sections[i].image = { ...agoda[i % agoda.length], alt: `${hotel.hotelName} ${sections[i].heading} 이미지` };
}
await writeFile(path, `// @ts-nocheck\nexport const generatedHotels: any[] = ${JSON.stringify(hotels, null, 2)};\n`);
console.log({ sanitized: hotels.length });
