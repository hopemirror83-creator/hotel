...
for await (const line of rows) {
  ...
  if (latitude < 13.65 || latitude > 13.87 || longitude < 100.40 || longitude > 100.68) continue;
  if (!/Bangkok|방콕|กรุงเทพ|Sukhumvit|수쿰윗|Siam|시암|Silom|실롬|Sathorn|사톤|Riverside|리버사이드|Khao San|카오산/i.test(address)) continue;
  const id = Number(row.hotel_id), name = clean(row.hotel_translated_name || row.hotel_name);
  if (!Number.isInteger(id) || id <= 0 || !name) continue;
  candidates.push({ slug: `bangkok-${id}`, hotelName: name, region: '태국 방콕', searchName: name, naverName: name, agodaHotelId: id, fallbackAddress: address, latitude, longitude, starRating: Number(row.star_rating) || undefined, reviewScore: Number(row.rating_average) || 0, reviewCount: Number(row.number_of_reviews) || 0, imageUrl: clean(row.photo1), landingUrl: `https://www.agoda.com/partners/partnersearch.aspx?pcs=1&cid=1927566&hid=${id}` });
}
candidates.sort((a, b) => b.reviewCount * b.reviewScore - a.reviewCount * a.reviewScore);
await writeFile('data/candidates-bangkok-v1-all.json', JSON.stringify(candidates, null, 2) + '\n');
console.log(`Selected ${candidates.length} Bangkok candidates`);
