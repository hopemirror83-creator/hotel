...
for await (const line of rows) {
  ...
  if (latitude < 18.72 || latitude > 18.86 || longitude < 98.90 || longitude > 99.08) continue;
  if (!/Chiang Mai|치앙마이|เชียงใหม่|Nimman|님만|Thapae|타패|Chang Klan|창클란|Mueang Chiang Mai/i.test(address)) continue;
  const id = Number(row.hotel_id), name = clean(row.hotel_translated_name || row.hotel_name);
  if (!Number.isInteger(id) || id <= 0 || !name) continue;
  candidates.push({ slug: `chiangmai-${id}`, hotelName: name, region: '태국 치앙마이', searchName: name, naverName: name, agodaHotelId: id, fallbackAddress: address, latitude, longitude, starRating: Number(row.star_rating) || undefined, reviewScore: Number(row.rating_average) || 0, reviewCount: Number(row.number_of_reviews) || 0, imageUrl: clean(row.photo1), landingUrl: `https://www.agoda.com/partners/partnersearch.aspx?pcs=1&cid=1927566&hid=${id}` });
}
candidates.sort((a, b) => b.reviewCount * b.reviewScore - a.reviewCount * a.reviewScore);
await writeFile('data/candidates-chiangmai-v1-all.json', JSON.stringify(candidates, null, 2) + '\n');
console.log(`Selected ${candidates.length} Chiang Mai candidates`);
