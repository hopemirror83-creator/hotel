...
for await (const line of rows) {
  ...
  const inBounds = latitude >= 12.25 && latitude <= 12.85 && longitude >= 99.82 && longitude <= 100.08;
  const localAddress = /Hua Hin|후아힌|หัวหิน|Cha.?am|차암|ชะอำ|Pranburi|프란부리|ปราณบุรี|Khao Takiab|카오타키압/i.test(address);
...
await writeFile('data/candidates-huahin-v1-all.json', `${JSON.stringify(candidates, null, 2)}\n`);
console.log(`Selected ${candidates.length} Hua Hin candidates`);
