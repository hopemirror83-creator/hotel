...
for await (const line of rows) {
  ...
  if (latitude < 8.55 || latitude > 8.95 || longitude < 98.18 || longitude > 98.38) continue;
  if (!/Khao Lak|카오락|카오락|Takua Pa|ตะกั่วป่า|Bang Niang|방니앙|Khuk Khak|쿡칵|Pak Weep|팍윕|Bang Sak|방삭|Phang Nga|팡아/i.test(address)) continue;
  if (/Phuket|푸껫|푸켓|Krabi|끄라비|Surat Thani|수랏타니/i.test(address)) continue;
  const id = Number(row.hotel_id), name = clean(row.hotel_translated_name || row.hotel_name);
  if (!Number.isInteger(id) || id <= 0 || !name) continue;
  candidates.push({ slug: `khaolak-${id}`, hotelName: name, region: '태국 카오락', searchName: name, naverName: name, agodaHotelId: id, fallbackAddress: address, latitude, longitude, starRating: Number(row.star_rating) || undefined, reviewScore: Number(row.rating_average) || 0, reviewCount: Number(row.number_of_reviews) || 0, imageUrl: clean(row.photo1), landingUrl: `https://www.agoda.com/partners/partnersearch.aspx?pcs=1&cid=1927566&hid=${id}` });
}
candidates.sort((a, b) => b.reviewCount * b.reviewScore - a.reviewCount * a.reviewScore);
await writeFile('data/candidates-khaolak-v1-all.json', JSON.stringify(candidates, null, 2) + '\n');
console.log(`Selected ${candidates.length} Khao Lak candidates`);
