...
for await (const line of rows) {
  ...
  if (latitude < 33.75 || latitude > 34.85 || longitude < 135.55 || longitude > 136.25 || !isNara(address)) continue;
...
await writeFile('data/candidates-nara-v1-all.json', `${JSON.stringify(candidates.slice(0, 1500), null, 2)}\n`);
console.log(`Selected ${Math.min(1500, candidates.length)} Nara candidates from ${candidates.length} coordinate and address matches`);
...
function isNara(value) {
  return /\b(?:Nara|Kashihara|Sakurai|Ikoma|Tenri|Yamatokoriyama|Yamato Koriyama|Yamatotakada|Yoshino|Uda|Gojo|Gose|Asuka|Ikaruga|Horyuji)\b|奈良|橿原|桜井|生駒|天理|大和郡山|大和高田|吉野|宇陀|五條|御所|明日香|斑鳩|法隆寺|나라|가시하라|사쿠라이|이코마|덴리|텐리|야마토코리야마|요시노|우다|고조|아스카|이카루가/i.test(value);
}
