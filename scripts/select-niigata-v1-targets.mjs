...
for await (const line of rows) {
  ...
  if (latitude < 36.65 || latitude > 38.65 || longitude < 137.55 || longitude > 139.95 || !isLocal(address)) continue;
  const id = Number(row.hotel_id);
  const name = clean(row.hotel_translated_name || row.hotel_name);
  if (!Number.isInteger(id) || id <= 0 || !name) continue;
  candidates.push({ slug: `niigata-${id}`, hotelName: name, region: '니가타현', searchName: name, naverName: name, agodaHotelId: id, fallbackAddress: address, latitude, longitude, starRating: Number(row.star_rating || 0) || undefined, reviewScore: Number(row.rating_average || 0), reviewCount: Number(row.number_of_reviews || 0), imageUrl: clean(row.photo1), landingUrl: `https://www.agoda.com/partners/partnersearch.aspx?pcs=1&cid=1927566&hid=${id}` });
}

candidates.sort((a, b) => rank(b) - rank(a));
await writeFile('data/candidates-niigata-v1-all.json', `${JSON.stringify(candidates.slice(0, 900), null, 2)}\n`, 'utf8');
console.log(`Selected ${Math.min(900, candidates.length)} Niigata candidates from ${candidates.length} coordinate and address matches`);
