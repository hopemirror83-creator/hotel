...
for await (const line of rows) {
  ...
  if (latitude < 12.77 || latitude > 13.00 || longitude < 100.82 || longitude > 100.99) continue;
  if (!/Pattaya|파타야|พัทยา|Jomtien|좀티엔|จอมเทียน|Na Kluea|Naklua|나끌루아|Sattahip|사타힙/i.test(address)) continue;
  const id = Number(row.hotel_id), name = clean(row.hotel_translated_name || row.hotel_name);
  if (!Number.isInteger(id) || id <= 0 || !name) continue;
  candidates.push({ slug: `pattaya-${id}`, hotelName: name, region: '태국 파타야', searchName: name, naverName: name, agodaHotelId: id, fallbackAddress: address, latitude, longitude, starRating: Number(row.star_rating) || undefined, reviewScore: Number(row.rating_average) || 0, reviewCount: Number(row.number_of_reviews) || 0, imageUrl: clean(row.photo1), landingUrl: `https://www.agoda.com/partners/partnersearch.aspx?pcs=1&cid=1927566&hid=${id}` });
}
candidates.sort((a, b) => b.reviewCount * b.reviewScore - a.reviewCount * a.reviewScore);
await writeFile('data/candidates-pattaya-v1-all.json', JSON.stringify(candidates, null, 2) + '\n');
console.log(`Selected ${candidates.length} Pattaya candidates`);
