...
for await (const line of rows) {
  ...
  if (latitude < 16.85 || latitude > 18.1 || longitude < 105.55 || longitude > 107.15 || !isRegion(address) || isOutlier(address)) continue;
  const id = Number(row.hotel_id);
  const name = clean(row.hotel_translated_name || row.hotel_name);
  if (!Number.isInteger(id) || id <= 0 || !name) continue;
  candidates.push({ slug: `quangbinh-${id}`, hotelName: name, region: '베트남 꽝빈', searchName: name, naverName: name, agodaHotelId: id, fallbackAddress: address, latitude, longitude, starRating: Number(row.star_rating) || undefined, reviewScore: Number(row.rating_average) || 0, reviewCount: Number(row.number_of_reviews) || 0, imageUrl: clean(row.photo1), landingUrl: `https://www.agoda.com/partners/partnersearch.aspx?pcs=1&cid=1927566&hid=${id}` });
}

candidates.sort((a, b) => (b.reviewCount * b.reviewScore) - (a.reviewCount * a.reviewScore));
await writeFile('data/candidates-quangbinh-v1-all.json', `${JSON.stringify(candidates.slice(0, 3000), null, 2)}\n`);
console.log(`Selected ${Math.min(3000, candidates.length)} Quang Binh candidates from ${candidates.length} matches`);

function clean(value) { return String(value || '').replace(/\s+/g, ' ').trim(); }
function isRegion(value) { return /Quang Binh|Quảng Bình|꽝빈|Dong Hoi|Đồng Hới|동허이|Phong Nha|퐁냐|Bo Trach|Bố Trạch|바돈|Ba Don/i.test(value); }
function isOutlier(value) { return /Quang Tri|Quảng Trị|꽝찌|Hue|Huế|후에|Vinh|Nghe An|Nghệ An|빈|응에안/i.test(value); }
