...
for await (const line of rows) {
  ...
  if (latitude < 14.15 || latitude > 15.55 || longitude < 108.35 || longitude > 109.25 || !isRegion(address) || isOutlier(address)) continue;
  const id = Number(row.hotel_id);
  const name = clean(row.hotel_translated_name || row.hotel_name);
  if (!Number.isInteger(id) || id <= 0 || !name) continue;
  candidates.push({ slug: `quangngai-${id}`, hotelName: name, region: '베트남 꽝응아이', searchName: name, naverName: name, agodaHotelId: id, fallbackAddress: address, latitude, longitude, starRating: Number(row.star_rating) || undefined, reviewScore: Number(row.rating_average) || 0, reviewCount: Number(row.number_of_reviews) || 0, imageUrl: clean(row.photo1), landingUrl: `https://www.agoda.com/partners/partnersearch.aspx?pcs=1&cid=1927566&hid=${id}` });
}

candidates.sort((a, b) => (b.reviewCount * b.reviewScore) - (a.reviewCount * a.reviewScore));
await writeFile('data/candidates-quangngai-v1-all.json', `${JSON.stringify(candidates.slice(0, 3000), null, 2)}\n`);
console.log(`Selected ${Math.min(3000, candidates.length)} Quang Ngai candidates from ${candidates.length} matches`);

function clean(value) { return String(value || '').replace(/\s+/g, ' ').trim(); }
function isRegion(value) { return /Quang Ngai|Quảng Ngãi|꽝응아이|Sa Huynh|Sa Huỳnh|사후인|Ly Son|Lý Sơn|리선|Binh Son|Bình Sơn|Dung Quat|Dung Quất/i.test(value); }
function isOutlier(value) { return /Quang Nam|Quảng Nam|꽝남|Da Nang|Đà Nẵng|다낭|Binh Dinh|Bình Định|빈딘|Kon Tum|꼰뚬/i.test(value); }
