...
import { getKrabiArea } from './krabiAreaGuides';
export function getKrabiSearchIntent(hotel: Hotel) {
  if (!hotel.slug.startsWith('krabi-')) return;
  const name = hotel.hotelName.trim(), area = getKrabiArea(hotel);
  const terms = area === '라일레이' ? ['끄라비', '라일레이', '이동', '조식'] : area === '끄라비타운' ? ['끄라비', '시내', '교통', '체크인'] : ['끄라비', area, '해변', '조식'];
  const title = `${name} 후기 모음 ${terms.join(' ')}`;
  return { title, seoTitle: `${title} | 예약 전 FAQ`, metaDescription: `${name}의 ${area} 위치와 공개 후기, 객실·조식·이동 조건을 예약 전 확인하도록 정리했습니다.`, intentChips: terms, recommendedFor: [`${area} 일정을 계획하는 여행자`, '숙소 위치와 후기 단서를 함께 비교하는 여행자'], notRecommendedFor: [area === '끄라비타운' ? '해변까지 도보 이동을 원하는 여행자' : '끄라비 시내 업무 동선만 우선하는 여행자'], faqs: [
